...
class SimCTGOPT(nn.Module):
    ...
    def beam_search(self, input_ids, beam_width, decoding_len, end_of_sequence_token_id = None, early_stop = False, speedup=True):
        if early_stop:
            try:
                assert end_of_sequence_token_id != None
            except AssertionError:
        ...
        output = self.model.generate(
                            ...
                            num_beams=beam_width, use_cache=use_cache)
        output = output[0]
        if early_stop:
            tmp = []
            for idx in range(len(output)):
